...
Just `#include <kahan-float/kahan.hpp>`, a single header library (using `-Iinclude/`). Or just get it from [include/kahan-float/kahan.hpp](include/kahan-float/kahan.hpp).
...
Just copy `include/kahan-float/kahan.hpp` to your project (or also `include/kahan-float/neumaier.hpp` if you prefer that).
