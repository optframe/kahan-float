...
(export CXX=/usr/bin/g++ && cd thirdparty && cd gbenchmark && rm -rf build && mkdir build && cd build && $CMAKE -DBENCHMARK_DOWNLOAD_DEPENDENCIES=ON -DCMAKE_BUILD_TYPE=Release .. && make )
