...
#ifdef HEADER_ONLY
#include <catch2/catch_amalgamated.hpp>  // HEADER_ONLY
#else
...
TEST_CASE("Kahan Tests 20x float32 0.1 <> 2.0") {
  float f = 0.0f;
  for (unsigned i = 0; i < 20; i++) f += 0.1f;
  REQUIRE(f != 2.0);
}
...
TEST_CASE("Kahan Tests infinity kfloat32 kfloat64") {
  ...
  // =============== ONE PLUS INF =================
  {
    ...
    REQUIRE(fm1 == std::numeric_limits<float>::infinity());
    REQUIRE(ffm1 == std::numeric_limits<double>::infinity());
    REQUIRE(kffm1.getValue() ==
            (double)std::numeric_limits<kfloat64>::infinity());
...
TEST_CASE("Kahan Tests basic some numeric_limits") {
  REQUIRE(std::numeric_limits<kfloat32>::lowest() ==
          std::numeric_limits<float>::lowest());
  REQUIRE(std::numeric_limits<kfloat32>::min() ==
          std::numeric_limits<float>::min());
  REQUIRE(std::numeric_limits<kfloat32>::max() ==
          std::numeric_limits<float>::max());

  REQUIRE(std::numeric_limits<kfloat64>::lowest() ==
          std::numeric_limits<double>::lowest());
  REQUIRE(std::numeric_limits<kfloat64>::min() ==
          std::numeric_limits<double>::min());
  REQUIRE(std::numeric_limits<kfloat64>::max() ==
          std::numeric_limits<double>::max());

  REQUIRE(std::numeric_limits<kfloat128>::lowest() ==
          std::numeric_limits<long double>::lowest());
  REQUIRE(std::numeric_limits<kfloat128>::min() ==
          std::numeric_limits<long double>::min());
  REQUIRE(std::numeric_limits<kfloat128>::max() ==
          std::numeric_limits<long double>::max());

  // max 32 < max 64 < max 128
  REQUIRE(std::numeric_limits<kfloat32>::max() <
          std::numeric_limits<kfloat64>::max());
  REQUIRE(std::numeric_limits<kfloat64>::max() <
          std::numeric_limits<kfloat128>::max());
}
...
TEST_CASE("Kahan Tests neumeier basic [1,10^100, 1, -10^100]") {
  ...
  //
  // Raymond Hettinger, Recipe 393090: Binary floating point summation accurate
  // to full precision, Python implementation of algorithm from Shewchuk (1997)
